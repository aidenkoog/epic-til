...
1. 서버리스 컴퓨팅(Serverless Computing) 개념
	•	**서버리스 컴퓨팅(Serverless Computing)**은 애플리케이션을 실행하기 위해 서버를 직접 관리할 필요 없이, 클라우드 제공자가 자동으로 서버 리소스를 할당하고 실행하는 방식을 의미함.
	•	FaaS(Function as a Service) 형태로 제공되며, 대표적인 서비스로 AWS Lambda, Google Cloud Functions, Azure Functions 등이 있음.
	•	개발자는 비즈니스 로직(코드) 작성에 집중하고, 서버 인프라 관리(배포, 확장, 유지보수)는 클라우드 서비스 제공자가 자동 처리.

✅ 서버리스와 기존 서버 방식 비교

항목	기존 서버 기반 컴퓨팅	서버리스 컴퓨팅
서버 관리	개발자가 직접 서버 구축 및 유지보수	클라우드 제공자가 자동 관리
자동 확장(Scaling)	수동 설정 필요	자동 확장
비용	상시 실행 비용 발생	사용한 만큼만 과금 (Pay-as-you-go)
배포 속도	인프라 설정이 필요하여 느림	빠른 배포 가능
운영 복잡성	서버 유지보수 필요	운영 부담 감소
...
장점	설명
서버 관리 불필요	인프라 관리 부담 감소, 개발자가 코드 작성에 집중 가능
비용 절감 (사용량 기반 과금)	사용한 만큼만 요금 지불 (Idle 시 비용 없음)
자동 확장 (Auto Scaling)	트래픽 증가에 따른 자동 확장
빠른 배포 및 업데이트	코드 수정 즉시 반영 가능
고가용성 및 장애 복구	클라우드 제공자가 자동 복구
이벤트 기반 실행	필요할 때만 실행 (예: 파일 업로드, HTTP 요청)

✅ 서버리스 컴퓨팅이 적합하지 않은 경우
	•	장시간 실행이 필요한 애플리케이션 (Lambda는 최대 실행 시간이 15분)
	•	고정적인 컴퓨팅 리소스가 필요한 애플리케이션 (예: 머신러닝 훈련, 대형 데이터 처리)

➡ 결론: 서버리스 컴퓨팅은 비용 절감, 자동 확장, 빠른 배포 등의 장점이 있지만, 모든 애플리케이션에 적합한 것은 아님. 따라서 트래픽 변동이 크거나 이벤트 기반으로 실행되는 서비스에 적합한 솔루션!
