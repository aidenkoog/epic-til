...
- Git과 SVN의 차이점
  - 개념
    - 형상 관리 방식
      - Git: 분산형 버전 관리 시스템 (DVCS, Distributed VCS)
      - SVN: 중앙 집중형 버전 관리 시스템 (CVCS, Centralized VCS)
    - 저장소 구조
      - Git: 로컬 저장소 + 원격 저장
      - SVN: 중앙 저장소
    - 인터넷 연결 필요 여부
      - Git: 로컬에서 모든 작업 가능 (인터넷 연결 필요 없음)
      - SVN: 중앙 서버와 연결 필요 (서버 다운 시 작업 제한)

  - Git과 SVN의 장단점
    - Git의 장점
	    - 로컬 저장소 지원 → 인터넷 없이도 커밋 가능
	    - 빠른 속도 → 로컬에서 모든 작업 가능
	    - 강력한 브랜치 관리 → 브랜치 생성 및 병합이 간편
	    - 오픈소스 및 무료 서비스 많음 → GitHub, GitLab, Bitbucket 등 지원

    - Git의 단점
	    - 초기 학습 난이도 높음 → 분산형 구조로 인해 개념이 복잡
	    - 대용량 파일 처리 어려움 → Git LFS(대용량 파일 지원)를 사용해야 함

    - SVN의 장점
	    - 중앙 집중형으로 보안 관리 용이 → 권한 관리 및 백업이 쉬움
	    - 파일 단위 체크아웃 가능 → 필요한 파일만 받을 수 있음
	    - 초보자가 사용하기 쉬움 → Git보다 단순한 명령어 구조

    - SVN의 단점
	    - 서버 다운 시 작업 불가능 → 중앙 서버 의존성이 큼
	    - 브랜치 관리 어려움 → 브랜치 생성과 병합 속도가 느림
	    - 속도가 느림 → 네트워크를 통해 모든 작업을 수행해야 함
