...
주소 표현	10진수, 점(.) 구분 (예: 192.168.1.1)	16진수, 콜론(:) 구분 (예: 2001:db8::1)
주소 개수	약 43억 개	사실상 무한대(2¹²⁸)
NAT 필요 여부	필요 (공인 IP 부족)	불필요 (충분한 IP 제공)
자동 주소 할당	DHCP 사용	SLAAC, DHCPv6 지원
보안	기본적으로 취약, IPsec 선택적 지원	IPsec 기본 지원
라우팅 속도	비교적 복잡	헤더 단순화로 속도 향상
이전 호환성	기존 네트워크 장비와 완벽 호환	IPv4와 직접 호환되지 않음
