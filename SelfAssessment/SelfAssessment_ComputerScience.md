...
- HTTPS와 SSL Pinning
  - HTTPS
    ...
    - HTTPS 동작 과정
	    - 클라이언트(브라우저)가 서버에 HTTPS 요청
	    - 서버가 SSL/TLS 인증서를 클라이언트에 전달
	    - 클라이언트가 인증서를 검증 (CA 신뢰 여부 확인)
	    - 암호화된 세션 키(대칭키) 교환 후 안전한 통신 시작
  ...
  - SSL Pinning
    ...
    - SSL Pinning 동작 과정
	    - 앱이 서버의 SSL 인증서의 공개 키(Public Key)를 앱 내부에 저장
	    - 앱이 HTTPS 요청을 보낼 때 서버의 인증서를 받아 내부 저장된 키와 비교
	    - 일치하면 정상적인 서버로 판단하고 통신 진행, 불일치하면 차단
  ...
  - 결론
	  - HTTPS만으로도 보안이 가능하지만, SSL Pinning을 적용하면 MITM 공격을 더 강력하게 차단할 수 있음.
	  - SSL Pinning은 보안성이 높지만, 인증서 변경 시 앱 업데이트가 필요하므로 운영 부담이 증가함.
	  - 보안이 중요한 금융, 결제, 개인정보 관련 앱에서는 SSL Pinning 적용이 권장됨.
