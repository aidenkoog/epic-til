...
3. 지도학습 vs. 비지도학습 차이점 정리

비교 항목	지도학습 (Supervised Learning)	비지도학습 (Unsupervised Learning)
데이터	입력(X)과 출력(Y)이 존재 (정답 있음)	출력(Y) 없이 입력(X)만 존재
목표	입력 → 출력 관계 학습 (예측)	데이터 내 숨겨진 패턴 탐색 (그룹화)
라벨(Label)	존재 (지도 데이터)	없음 (라벨 없는 데이터)
주요 알고리즘	회귀, 분류 알고리즘 (SVM, NN, 의사결정나무 등)	클러스터링, 차원 축소 (K-Means, PCA 등)
활용 예시	이메일 스팸 필터, 이미지 인식, 주가 예측	고객 세분화, 이상 탐지, 추천 시스템
장점	예측 정확도가 높음	새로운 패턴 탐색 가능
단점	라벨링 비용이 높음, 데이터 의존성 큼	정답이 없어 해석이 어려움

4. 지도학습과 비지도학습의 결합 (반지도학습, 강화학습)
...
➡️ 지도학습이 예측을 위한 모델이라면, 비지도학습은 패턴을 찾는 모델!
