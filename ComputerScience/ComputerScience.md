...
- 개념 / 용어 정의, 사용 이유
- 컴퓨터 기본 지식, 컴퓨터 구조, 운영체제 등의 컴퓨터 전반의 지식
...
- 특별한 정보
  - Host
    ...
    - 하나의 아이피에 여러 가상 도메인이 있을 수 있다
    - 호스트 정보가 있어서 가상 호스팅 환경에서 도메인 구분이 가능
  - Location
    - 3xx 응답 결과, 자동 리다이렉트
    - 201 (Created) 생성된 리소스 URI
    - 요청을 자동으로 리다이렉션하기 위한 대상 리소스
  - Allow
...
- 쿠키
  - Set-Cookie: 서버에서 클라이언트로 쿠키 전달 (응답)
    - 응답 시 Set-Cookie; user=xxx
    - 웹 브라우저에는 저장소가 존재, 쿠키 저장소에 user=xxx를 저장
    - set-cookie; sessionId=xxx; expires=xxx ...
  - Cookie: 클라이언트가 서버에서 받은 쿠키를 저장하고 HTTP 요청 시 서버로 전달
    - 로그인 이후에 다시 페이지 접근 시 쿠키를 먼저 검색 후 쿠키 존재 시 Cookie; user=xxx 셋팅
  - 재정리
    - HTTP 무상태 프로토콜, 요청,응답을 주고 받으면 연결끊어짐, 상태 유지하지 않음
  - 사용처
    - 사용자 로그인 세션 관리 (이름을 그대로 보내면 위험)
    - 광고 정보 트래킹
  - 쿠키 정보는 항상 서버에 전송
    - 네트워크 트래픽 추가 유발
    - 최소한의 정보만 사용 (세션 아이디, 인증 토큰)
    - 서버 전송 없이 클라이언트에 들고 있고 싶다면
      - 웹 브라우저 내부에 데이터를 저장하고 있으면 웹 스토리지 사용 (localStorage, sessionStorage)
  - 보안 민감 데이터는 저장하지 않는것 권장
  - 쿠키 생명주기
    - Expires
    - max-age (초단위 셋팅 0이나 음수 지정시 쿠키 삭제)
    - 세션 쿠키: 만료 날짜 생략 시 브라우저 종료시까지만 유지
    - 영속 쿠키: 만료 날짜를 입력하면 해당 날짜까지 유지
  - 쿠키 - 도메인
    - 도메인을 지정해 쿠키 생성 (아무 사이트나 다 생성되면 안되니까)
  - 쿠키 - 경로
    - 하위 페이지 경로만 쿠키 접근
    - 일반적으로 path=/ root로 지정
  - 쿠키 - 보안
    - Secure: https인 경우에만 클라이언트에서 서버로 전송
    - HttpOnly
      - XSS 공격 방지
    ...
    - SameSite
      ...
      - 다른 사이트에서 쿠키 생성해서 사용하는 위험 방지

#### HTTP 헤더 - 캐시와 검증 헤더와 조건부 동작

- 캐시
  - 응답에 cache-control: max-age=60 60초 동안은 캐시 유효
  - 응답 결과를 브라우저 캐시에 저장 (60초 유효)
  - 두번 째 요청 시 캐시 먼저 확인 (로딩 속도 빠르게 가능)
  - 세번 째 요청 시 캐시 확인
  - 캐시 시간 초과
  - 다시 서버로 요청
  - cache-control: max-age=60
  - 브라우저 캐시 갱신
  - 그런데 만약에 요청해서 받는 이미지 정보가 항상 같다면?
- 캐시 유효 시간 초과해서 다시 요청시 상황
  - 기존 데이터 변경
  - 기존 데이터 동일
    - 클라이언트의 데이터와 서버 데이터가 같다는 사실을 확인할 수 있는 방법 필요
    - 그것이 검증 헤더
    - 검증헤더인 Last-Modified (UTC 표기법) 추가, cache-control과 더불어
    - 응답받으면 Last-modified 정보도 같이 저장
    - 캐시 시간 초과 시 다시 요청하는데 조건부 요청인 if-modified-since를 설정해서 요청
    - 서버에서 봤더니 최종 수정일이 같다면?
    - 304 Not Modified 응답 (HTTP Body 없음), Last-Modified, cache-control은 있음
    - 헤더 메타 정보만 클라이언트에 전송
    - 304이므로 cache-control 갱신 후 캐시에서 데이터 읽어들임
    - 네트워크 다운로드 발생 하나 용량이 적은 헤더 정보만 다운로드
    - (참고) 크롬 인스펙터에서 색이 연한것은 캐시에서 받아온 것을 의미
...
- 조건부 요청헤더
  ...
  - If-Modified-Since: Last-Modifed 사용
    - 1초 미만 단위로 캐시조정 불가능
    - 날짜 기반의 로직 사용 불가피
    - 데이터는 같은데 날짜가 다른 경우 전체데이터 다시 다운로드하는 비효율적인 절차 발생
  - If-None-Match: ETag 사용
    - Entity Tag
    - 캐시용 데이터에 임의의 고유한 버전 이름 달아둠
    - 데이터 변경 시 이 이름을 바꾸어서 변경함 (해시 다시 생성)
...
#### PG 사 관련 MID, KEY 정보 설명

- MID
  - 신용카드 상점 ID = MID = 가맹점 번호
  - MID: 가맹점 식별 아이디 의미
    - 나이스 페이 예: CV로 시작하는 값
    - 발급 절차
      - MID 발급 —> 등록비 결제를 요청하는 스텝 —> 등록비 결제 —> 해당 관리자 사이트로 이동 —> MID에 할당된 키 값 확인
      - 나이스 페이 Key 값 예: 나이스페이 가맹점 관리자 사이트 내에서 확인한 값
- KEY
  - 보통 API 호출 시 필요한 헤더값으로 사용
  - 진흥원에 가맹점 등록 후 발급 (MID 발급 --> 해당 사이트 접속 --> 키 정보 확인)

#### 대사 작업 또는 대사 파일 설명

- 대사 작업 = 정산 대사 작업
- 대사 파일 = 정산 파일
- 매출 금액과 정산이 되어 들어온 금액을 확인하는 작업
- 보통 결제회사 (한국 사이버 결제)에서 정산 내역을 파일로서 제공
- PG (대행 결제)를 사용하는 경우 정산을 대행하여 처리

#### Fallback 에 대한 이해

- 어떤 기능이 제대로 동작하지 않을 때, 이에 대처하는 기능 또는 데이터 또는 동작 의미
- API 호출 시 발생하는 예외처리를 위한 클래스를 정의
- 실패에 대해서 후처리를 위해 설정해 두는 메소드
- 다음 경우에 원래 동작 대신 호출
...
- HTML5 표준의 일부로 개발됨 (클라이언트와 서버 간의 실시간 양방향 통신 가능케 함)
- 싱글 TCP 소켓에서 통신이 이루어짐
- ws (비안전) , wss (안전)
- 최초 연결은 일반 HTTP 요청을 통해 이루어지며 이 과정 내 핸드쉐이킹 발생
- 기존 80, 443 포트 사용하므로 추가적인 방화벽 설정 없이도 통신 가능
- STOMP 는 메시징 전송을 더 쉽게 하기 위해 개발된 프로토콜, 웹소켓 위에서 작동
- 특정 아이피 방화벽 이슈 존재했었음
  - 특정 서버 간에 접속이 허용되어 있는 확인 필요
  - 랜선 연결 시 와이파이로 전환 시도
  - 클라이언트에서 해당 아이피로 핑 테스트
- hop-by-hop 프로토콜
  - 프록시 서버에서 가로챈 요청에 대해 웹 소켓 연결이라는 정보를 추가로 명시해야 한다?

#### 동시성, 병렬성

- 동시성 (Concurrency) : 일을 여러개로 나누어 번갈아 가면서 실행하여 동시에 처리하는 것처럼 보이는 것
- 병렬성 (Parallelism) : 멀티코어 환경에서 실제로 여러개의 일을 동시에 처리하는 것

#### 페이먼트 tid

- TID(Transaction ID)는 결제 시, 개별 거래 건마다 고유하게 생성되는 거래 번호
- TID 예시 : xxxpay00m01011705011200000123

#### 소켓 재정리

- 소켓은 네트워크 상에서 돌아가는 두 개의 프로그램 간 양방향 통신의 엔드 포인트.
- 앤드 포인트
  - 여기에서의 앤드 포인트는 아이피 주소와 포트 번호의 조합을 의미함. 
...
- %02X: 헥사를 두자리로 변환시키는 포맷
- String.format은 성능상 좋지 않음
- 비트 연산의 경우 CharArray + Shift 방식이 다소 성능 높음
- 바이트 배열을 스트링으로 변환
  - bytes.toString(CharSets.UTF-8)
- FF, ff 서로 다른 값
- %02X의 경우 대문자(ex 1A5F), %02x의 경우 소문자(ex 1a5f)

#### WOL

- WOL (Wake On Lan)은 PC를 원격으로 켜는 기능
- PC를 언제 어느곳에서나 원격으로 필요한 순간에만 켜서 사용하기 위해 필수적인 기능
- PC가 반드시 유선랜에 연결되어 있어야 함
- 무선랜으론 아직 구현이 되지 않고 있음
- 아마도 전기적인 신호를 물리적으로 주어야하기 때문
- 무선랜을 통해서 PC를 깨우려면 무선으로 들어오는 매직 패킷 신호를 감지하기 위해 PC의 회로가 계속 깨어있어야 함
- PC의 메인보드 및 네트워크 카드가 WOL을 지원 필수
- 다음의 설정이 필요
...
- Authorization 헤더는 HTTP 프로토콜에서 클라이언트가 서버에 인가된 상태임을 전달하는 수단으로 사용
- 이는 사용자 인증을 거친 후, 서버에 자신의 신원을 증명하기 위해 필수적
- 일반적으로 다음과 같은 구조 형태
  - Authorization: <스키마> <자격 증명>
  - 여기서 <스키마>는 인증 방식(예: Basic, Bearer)을 나타내고, <자격 증명>은 클라이언트의 인증 정보를 포함
  - Bearer는 이러한 인증 방식 중 하나로, OAuth 2.0에서 널리 사용되는 스키마

#### Bearer 토큰이란?

- Bearer 토큰은 클라이언트가 특정 리소스에 접근할 수 있는 권한을 증명하기 위해 서버에 전달하는 토큰
- 토큰은 HTTPS를 통해 전송되어야 하며, 이를 통해 토큰이 네트워크에서 탈취되는 것을 방지할 수 있음
  - Authorization: Bearer <토큰>

#### Threshold (쓰레스홀드, 임계값)

- 무엇인가가 일어나기 시작하는 기준점 또는 경계선
- 소리가 일정 크기(Threshold)를 넘어서면 귀에 들리기 시작하는데 이 최소한의 크기가 Threshold
- 점수 60점을 넘어야 합격이라면 60점이라는 수치가 Threshold
- 결론: 무언가가 시작되거나 변화하는 기준점, 설정된 기준값을 넘으면 새로운 상태가 시작되고, 넘지 못하면 기존 상태를 유지

#### GET, POST 개념, 데이터 흐름 설명

- GET과 POST는 웹 서버와 클라이언트 간 데이터를 교환하는 HTTP 메서드입니다
- GET은 주로 서버에서 정보를 조회하기 위해 사용하며, 요청하는 데이터는 URL의 쿼리 문자열에 포함되어 전송됩니다.
- 이로 인해 URL에 데이터가 노출되며, 크기 제한이 있습니다. 
- POST는 데이터를 서버에 제출할 때 사용되며, 데이터는 요청 본문에 포함됩니다.
- 따라서 데이터의 크기 제한이 없으며, 민감한 정보 전송에 적합합니다.
- GET은 데이터 조회에, POST는 데이터 제출 및 변경에 주로 사용됩니다.
- 데이터 흐름에서 GET은 서버에서 클라이언트로, POST는 클라이언트에서 서버로 주로 흘러갑니다.

#### 쿠키, 세션, 웹스토리지의 차이 설명

- 쿠키, 세션, 그리고 웹 스토리지는 모두 클라이언트와 서버 사이에서 정보를 저장하고 교환하는 방법들
- 각각 목적과 작동 방식에 차이가 있습니다
- 쿠키는 클라이언트 측에 저장되는 작은 데이터 조각으로, 사용자의 상태 정보나 ID를 저장하여 웹 서버가 사용자를 식별할 수 있게 해줍니다.
- 그러나, 쿠키는 보안에 취약하며, 저장할 수 있는 데이터 크기에 제한이 있습니다.
- 세션은 서버 측에서 관리되며, 클라이언트의 상태 정보를 서버에 저장합니다.
- 클라이언트는 세션 ID만 쿠키로 저장하고, 이를 통해 서버는 클라이언트를 식별합니다.
- 이 방식은 쿠키보다 보안에 강하고, 데이터 제한이 적습니다.
- 웹 스토리지는 HTML5에 도입된 기술로, 로컬 스토리지와 세션 스토리지로 나뉩니다. 
  - 로컬 스토리지는 데이터를 클라이언트의 브라우저에 영구적으로 저장하며,
  - 세션 스토리지는 브라우저 세션이 종료되면 데이터가 삭제되는 특성을 가집니다.
...
- CPU 구성요소
  - 연산장치 및 제어장치
  - 레지스터
  - 버스: 내부버스, 외부버스(데이터, 제어, 주소)
- 주요 레지스터
  - MAR, MBR, IR, PC
- CPU 아키텍처
  - Von Neuman(폰 노이만): 명령과 데이터가 같은 버스(CISC 명령구조)
  - Havard: 명령과 데이터가 별도 버스 (RISC 명령구조)
- 명령집합 종류
  - CISC: 필요한 모든 명령어, 복합 명령집합, 명령 세트가 복잡, 가변명령어
