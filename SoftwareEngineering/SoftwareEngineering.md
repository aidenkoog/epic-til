...
## Coding, 자바 코드 변환: 실제 구현할 수 있는 모델 내용 재정리

- Product Line
  - Core Asset을 사용해서 소프트웨어를 개발하는 방법, 소프트웨어를 재사용하는 방법 (Core Asset: 업무 공통 컴포넌트)
  - 도메인 공통 컴포넌트를 사용해서 소프트웨어를 개발하는 프로세스
  - 종류
...
- 종류
  - Verification (기능 위주/초점)
  - Validation (행위 위주/초점, 고객 관점)
- 단계
  - 단위테스트
    - 모듈 테스트
    - 기법: 화이트박스, 블랙박스 (단위, 통합, 시스템, 인수 모두에 사용 가능한 기법)
      - 화이트박스 기법: 구조, 루프테스트
      - 블랙박스 기법: 균등분할, 경계값, 오류예측, 원인결과
  - 통합테스트
    - 인터페이스 테스트
    - 종류/기법: 빅뱅, 탑-다운, 바텀-업, 샌드위치
    - 프로그램 필요
    - 프로그램
      - Stub (꼬리)
      - Driver (머리)
  - 시스템 테스트
    - 성능, 보안, 가용성, UI
      ...
      - 가용성: FailOver 테스트
        - *Failover
          - 장애조치 기능, 시스템 장애 발생 시 하나 이상의 예비 백업 시스템(노드)로 자동 전환되는 것
          - 장애 극복 기능, 컴퓨터서버, 시스템, 네트워크 등 이상 발생 시 예비 시스템으로 자동전환 기능
    - 비기능적 요구사항에 대한 테스트
  - 인수테스트
    - Validation 알파 베타
  - 테스트 절차
    - 테스트 환경 구축, 테스트 시나리오, 테스트케이스 작성
  - 결함 발생
    - 결함 관리, 결함 관리 추적 보고서 작성
  - 소프트웨어 테스트 설계 기법
    - 명세기반(블랙박스)
    - 구조기반(화이트박스)
    - 경험기반
...
- 애자일 프로세스
  - 변화에 능동적으로 대응하기 위한 민첩한 프로세스
  - 가변적인 요구사항에 대응하기 위해서 사람 중심의 프로세스
- 스크럼 (SCRUM)
  - 팀 단위 활동을 강조한 애자일 방법론
  - 특징: 변화에 민첩하게 대응 가능
  - 구성 요소
    - Product Backlog
    - Sprint Backlog
    ...
    - Daily Scrum
- 스크럼 팀구성
  - 프로덕트 오너 (비전, 요구사항 제시, 우선순위 결정)
    - 중요도, 난이도로 우선순위 결정
  - 스크럼 마스터 (프로덕트 오너와 팀 멤버간의 조율 역할)
  - 스크럼 팀 (2~3주 단위로 개발 진행)
- XP (eXtreme Programming)
  - 테스트를 중요시하는 경량화된 방법론/애자일 방법론
  - 4가지 가치
    - 단순성
    - 의사소통
    - 빠른 피드백
    - 용기 존중
  - 12개의 실천항목 존재 (12 Practices)
    - TDD
    - Pair Programming
    - Refactoring
    - OnSite Customer
- TDD (Test Driven Development)
  - 코드 리팩토링 + 테스트를 반복하는 방법론
  - JUnit + Espresso 로 테스트 반복

## Software Quailty 재정리

- Software Quality Management: 요구사항과의 일치성을 관리/확인하는 활동
  - 구성요소 / 종류
    - Software Quality Planning (품질 관리에 대한 목표, 범위, 일정, 평가/검수 기준, 비용 정리)
    - Software Quality Assurance (Inspection, 프로세스, 품질 보증 절차 작성, Formal Review로)
    - Software Quality Control (테스트 프로덕트, 완성된 프로덕트를 테스트, 단위, 통합, 인수)
- 소프트웨어 품질의 문제점/특징
  - 주관적 (이해관계자, 당사자 간 서로 다른 견해)
  - 정량적 측정 어려움
  - 시간에 따라 변화한다.
- 품질 관점(Quailty View)
  - 발주자: 경제성, 비용대비 효과, Cost and Benefit
  - 사용자: 기능 완전성
  - 개발자: 개발 표준과 복잡성
  - 유지보수자: 이해용이성
- 품질 매트릭스: 소프트웨어 품질을 정량적으로 측정하는 척도
  - 품질측정 대상
    - 프로젝트 진척률 평가/관리
    - 프로세스 표준화, 개발 프로세스 표준화 아래에서 개발 중인지 확인
    - 프로덕트, 만들어진 제품의 품질이 우수한가 (테스트 통해)
  - 품질측정 방법
    - 직접법: 코딩을 직접적으로 본다는 의미
      - LOC, FP, 회전 복잡도(모듈화의 척도, 폐구간 + 1), 과학적 매트릭스(연산자, 피연산자 대상으로 프로그램의 규모, 크기 측정)
    - 간접법: ~성으로 끝나는 기능성, 사용성 등
      - 기능성: 요구사항 다 개발한것인지
      - 사용성: 경험에 의한 편의성 존재하는가
      - 신뢰성
      - 유지보수성
- 품질 측정 절차: 분류, 정의, 식별
- PERT 기법
  - 정의
  - PERT 기법 통해 LOC 추정 가능
- FP
  - 정의: 사용자가 요구한 기능만 규모 산정 대상으로 한다.
- SPI
  - 소프트웨어 프로세스 개선
  - 종류
    - 프로세스
...
- 품질모델 (Quality Model)
  ...
  - ISO 품질관련 표준 모델
    - (Product) 제품관련 모델
      - ISO-9126 (품질특성 - 기능성, 사용성, 신뢰성, 유지보수성, 이식성)
      - ISO-14598 (절차)
      - GS(Good Software, 우리나라 것)
    - (Process) 프로세스 관련 모델
      - CMMi (단계형, 연속형 모델 제시)
        - 단계형
        - 연속형
...
- LOC (Line Of Code)
- COCOMO (Constructive Cost Model)
  - LOC 기반으로 산정하고 프로그램 크기에 따라 계산식 제공 (정해진 계산식)
  - 구조적, 정보공학 관점에서 사용
  - Coding Line 에 따라 계산식 상이
  - COCOMO 프로젝트 개발 유형
    - 유기적: 5만 라인 이하, 업무 프로그램
      - MM = 2.4(KDSI) * 1.05
    - 반 내장형: 30만 라인 이하, OS 및 DBMS
      - MM = 3.0 (KDSI) * 1.12
    - 내장형: 30만 라인 이상, 실시간 시스템, 미사일 유도, 제어시스템
      - MM = 3.6(KDSI) * 1.20
- COCOMO 2
  - 재사용 고려한 비용 예측
  - 객체지향에서 사용
- FP
  - 정의: 사용자 관점에서 기능을 정량적으로 측정
  - 산정 종류
    - 간이법(제안법) - 제안단계
    - 정규법(정통법) - 완료시점
- FP 구성요소
  - 데이터 기능점수 (INF, EIF)
  - 트랜잭션 기능 점수 (EI, EO, EQ)
    - External Input/Output/Query

## 정보시스템 감리 재정리

- 정보시스템 감리 정의
  - 독립된 사용자가 객관적으로 측정하는 것
- 감리시점에 따른 유형
  - 상시감리
  - 상주감리
- 의무 감리
