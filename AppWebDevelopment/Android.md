...
- 개념 / 용어 정의, 사용 이유
- 안드로이드 플랫폼, 앱 개발, UI개발 기술, 안드로이드 아키텍쳐 전반에 대한 정리
...
#### kotlin in, out, generic 재정리

- Generic:
  - 클래스, 인터페이스, 함수 등에서 동일한 코드를 재사용하고 싶을 때 여러 타입을 지원하기 위한 유용한 기능
  - 코틀린에서의 제네릭은 자바와 마찬가지로 타입 불변성을 가짐
    - 타입 불변성이란, '제네릭 타입을 사용하는 클래스, 인터페이스'에는 해당 타입의 자식이나 부모를 대입할 수 없고 오직 일치하는 타입만을 대입할 수 있는 것을 의미
  - Array는 값을 바꿀 수 있는 가변, List는 값을 바꿀 수 없는 불변 즉, 리스트이면 불변의 속성을 가지고 있으므로 부모 타입의 리스트 파라비터 매개변수에 자식 타입의 리스트 인자를 전달하는 것이 가능
  - 리스트와 어레이 정의를 보면 리스트 제네릭 타입 앞에 out 키워드가 붙어있음
- out
  - A가 B를 상속받아도 Class<A>는 Class<B>를 상속받지 않는다 라는 불변성의 원리로 인해 컴파일 에러를 발생
  - 이를 해결하기 위해서는 A가 B를 상속받으면 Class<A>는 Class<B>를 상속받는다 로 바꿔주어야 함. 공변성(Covariance)
  - Covariance로 변환하기 위해 사용하는 키워드가 바로 out
  - out 키워드를 from의 제네릭 타입에 적용하면
Array<Cat>은 Array<Animal>을 상속받게 되므로 주석 부분에서 에러가 발생하지 않음
  - 그리고 Nothing 타입을 write 하는 것으로 예측함
- in
  - write만 할 수 있고 read는 할 수 없는 것, 반공변성(Contravariance), in 키워드를 사용
  - A가 B를 상속받으면 Class<B>는 Class<A>를 상속
...
#### 메모리 릭 검사 빙법, 메모리 확인법 그리고 수정 예 정리

- 작업 순서
  - 메모리 릭을 테스트할 기능 / 화면 선정
  - 테스트할 기능 반복 수행
  - 안드로이드 스튜디오에서 메모리 확인
  - 메모리릭 수정
- 테스트할 기능 및 화면 선정 스텝
  - 앱 전체 기능을 한번에 테스트하고 메모리릭을 찾는 것은 매우 어려움
  - Static으로 선언되어 공용으로 사용되는 변수나 이벤트의 경우, 릭을 발견하더라도 어느 기능에서 릭이 발생했는지 명확한 포인트를 선별하기 어렵기 때문
  - 따라서 한 화면, 한 기능씩 테스트를 하고 릭 발생 여부를 체크해나가는 것이 중요
- 테스트할 기능 반복 수행
  - 테스트할 기능을 선정했다면, 앱을 debug 빌드후, 해당 기능을 반복적으로 수행
  - 이때 중요한 것은 해당 기능의 시작과 종료에 따라 명확히 메모리가 해제되는 지점을 기준으로 해야함
  - 예를 들어 Activity 라면, Activity 가 onCreate 된 후, onDestory 가 호출될 때 까지 반복테스트 해야하며, Activity 에서 선언된 변수와 callback 등이 모두 해제되는 기준으로 테스트
  - 앱 실행 - back key 로 화면 종료를 반복
  - Activity 가 종료되더라도 App의 Process는 남아있기 때문에, 다시 앱을 실행하더라도 동일한 process에서 Activity가 수행
  - 만약 Activity에 메모리 릭이 있다면 이전 실행에서 남아있던 메모리가 해제되지 않아 메모리 릭을 쉽게 발견할 수 있음
  - 주의 할 점은 테스트 중에 앱을 강제 종료하면 안됨. 강제 종료 시 앱에서 할당받은 메모리가 모두 해제되고, 재실행시 새로운 process로 시작되기에 메모리 릭을 발견할 수 없음
  - 따라서 메모리릭 체크를 처음한다면, 앱의 첫화면부터 진행하는 것 권장
  - 첫 화면에서 메모리 릭이 없다면, 다음 화면이나 기능을 하나씩 선정하여 메모리 릭 체크를 진행하면 됨
- 안드로이드 스튜디오에서 메모리 확인
  - Profiler - sessions 에서 분석할 앱 선택
  - 휴지통 아이콘 클릭해서 강제 gc 발생시킨 다음 깨끗한 상태에서 테스트 시작
  - 이 동작후 GC로도 해제되지 않은 메모리를 검출할 수 있음. 그런 다음 Capture heap dump를 선택하고 Record를 선택
  - Leaks 항목이 0 으로 뜨면 테스트한 화면/기능에서 메모리 릭이 없다는 의미
- 메모리 릭 수정
...
- 1. 서버와 연결할 때 사용할 socket 객체를 생성 (여기서 사용하는 socket 객체는 서버와 이벤트를 주고받는 역할)
- 2. 서버가 전송한 이벤트를 받았을 때 실행할 동작을 .on() 함수를 사용해서 설정
- 3. 서버에게 연결을 요청
- 4. 서버와의 연결에 성공하면 onConnect Listener 의 call 함수가 호출

#### PG 결제 모듈 연동

- 개발자를 위한 PG 결제 API (아임포트, 부트페이 등)
- 부트페이
  - BootPay.init
  - setPG (결제할 PG 사)
  - setMethod (결제 수단)
...
- 개념적인 부분들
  ...
  - 결제 준비
  - 결제 승인
  - 회원인증, 결제 수단 선택, 승인 번호 검증
  - 결제 취소 , 거래 결과 조회
  - 대사 파일 조회  
- 추가 설명
  - 신용 / 체크 카드 결제 수단 선택 화면이 있다고 가정
  - 개발자가 직접 결제를 구현하고 카드마다 각각 코드를 직접 짜줘야 하고 연동도 해줘야 한다면 힘든일.
  - 그래서 이를 중간에서 연동을 도와주는 회사가 존재
  - 이를 PG (Payment GateWay)
  - 많이 사용하는 회사들로는 주로 아임포트 또는 토스페이먼츠
  - 카드 정보 인증 > 카드 정보를 결제사 서버로 송신 > 결제사 서버에서 PG 사로 송신 > PG 사에서 카드회사로 결제 정보 송신
  - 결제 정보가 올바르면 카드 회사에서 응답 > 결제 성공

#### reified 키워드에 대한 고찰

- inline function 과 조합해서만 사용 가능
- 제네릭 타입은 런타임때 정보가 없어지는데 reified T 형식으로 쓰면 간단하게 사용 가능
...
- 서버와 클라이언트 둘다 자바로 코딩 되어 있으면 오브젝트로 송신해도 상관 없으나 서버가 C++등으로 구성된 경우에는 오브젝트로 보내면 안됨
- byte 단위로 송신해야 함
- 소켓 통신: 서버와 클라이언트 양방향 연결이 이루어지는 통신
- 서버 소켓 동작 순서 : socket - bind - listen - accept
- 클라이언트 소켓 동작 순서 : socket - connect
- 소켓 통신 시 바이트데이터가 0x00 이 있으면 이 데이터는 EOF로 간주
- Byte의 범위는?
...
- 시간이 걸리는 작업들이 여러개 있을 때 모든 작업이 완료가 되어야 await()가 위치한 코드가 동작
...
- USB OTG(On-The-Go)는 USB 디바이스 간에 직접 통신할 수 있도록 하는 표준
- OTG 호스트와 OTG 디바이스는 역할에 따라 구분
- 1. **OTG 호스트**: 일반적으로 스마트폰, 태블릿, 노트북 등이 OTG 호스트 역할
- OTG 호스트는 다른 USB 장치(예: USB 플래시 드라이브, 마우스, 키보드 등)와 통신 가능
- 이 장치는 USB 포트를 제공하고 다른 USB 장치를 인식하고 제어
- 2. **OTG 디바이스**: OTG 디바이스는 주로 USB 플래시 드라이브, 디지털 카메라, 외장 하드 드라이브 등이 해당
- OTG 디바이스는 호스트와 통신하고 호스트의 명령을 받아들이는 역할
- 따라서 OTG 디바이스는 호스트의 요청에 따라 데이터를 읽거나 쓸 수 있음
- 간단히 말하면, OTG 호스트는 다른 USB 장치와 통신할 수 있는 장치이고, OTG 디바이스는 호스트와 통신하고 명령을 받아들이는 USB 장치
- OTG 호스트와 디바이스는 필요에 따라 역할을 교환할 수도 있음 (툴박스)