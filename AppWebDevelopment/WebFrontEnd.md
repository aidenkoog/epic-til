...
- 어떤 버전의 HTML을 사용할 것인지 알려주는 태그

#### Javascript 에서 falsy에 대한 설명

- bool로 변환될 때 false 가 되는 값
...
- 비동기 웹 응용 프로그램을 만들기 위해 클라이언트 측에서 사용되는 웹 개발 기술 집합
- 사용 시 이점
  - 기존 페이지의 화면 / 동작을 방해하지 않으면서 백그라운드에서 비동기적으로 서버로 데이터를 보내고 서버에서 데이터를 받아오는 것이 가능
- 동작 원리
  - 프리젠테이션 레이어에서 데이터 교환 레이어를 분리함으로써, 웹페이지 / 확장 웹 앱이 전체 페이지를 다시 로드 할 필요 없이 동적으로 컨텐츠를 변경할 수 있도록 함.
  - 최근 트렌드는 XML 대신 JSON 사용
  - XMLHttpRequest API는 비동기 통신에 자주 사용, 최근에는 fetch API를 사용

#### package-lock.json 파일에 대한 설명

- 개발자들이 동일한 node_modules 트리를 생성해서 같은 의존성을 설치할 수 있도록 보장해주는 파일
- 해당 파일 내 정확한 패키지 버전명이 작성되어 있어 구체적인 정보가 있어서 아래 문제 발생을 방지
  - npm 버전 또는 사용하던 패키지 자체의 버전이 다르게 배포되었을 경우, 다른 node_modules 트리가 생성될 수 있음

#### CSRF나 XSS 공격을 막는 방법

- CSRF 공격을 막기 위해서는 서버에서 CSRF Token을 생성하여 세션에 저장하고, 프론트엔드에서 요청 시 해당 Token을 함께 전송하여 인증
- SameSite 속성을 쿠키에 설정하여 도메인이 다른 사이트에서는 쿠키를 사용할 수 없도록 제한하는 방법도 존재
- XSS 공격을 막기 위해서는 입력 값들을 유효성 검증하고, 특수문자들을 제외하는 정규식을 통해서 제거
- 서버에서 CSP(Content-Security-Policy)정책을 설정하여, 허용된 스크립트만 실행되도록 제한하는 것도 가능
- HTTP 대신에 신뢰할 수 있는 HTTPS를 사용하여 통신 프로토콜을 암호화하는 것도 가능

#### CSRF 설명

- Cross Site Request Forgery, 사이트 간 요청 위조
- 사용자가 자신 의도와는 무관하게 공격자가 의도한 행위(수정/삭제/등록 등)를 웹사이트에 요청하게 하는 공격

#### XSS 설명

- Cross Site Scripting, 사이트 간 스크립팅
- 공격하려는 사이트에 스크립트를 넣는 기법
- 쿠키나 세션 토큰 등의 민감한 정보 탈취
- 종류
  - Stored XSS
  - Reflected XSS
...
- sso: 여러 개 사이트에서 한 번의 로그인으로 여러 가지 다른 사이트들을 자동적으로 접속하여 이용하는 방법
- ldap: 네트워크 상의 자원을 식별하고 인가된 사용자만 접근하도록 하는 네트워크 디렉토리 서비스

#### undefined와 null 차이

- undefined: 변수를 선언했지만 값이 할당되지 않은 변수
- null: 명시적으로 할당, 값을 나타내진 않지만 명시적으로 할당했다는 점에서 undefined와 다름

#### Redux, Recoil 비교

- Redux
  - 단일 스토어를 가지므로 디버깅에 용이
  - Redux devtools와 같은 디버깅 툴 존재
  - React 라이브러리가 아니므로 React의 내부 스케쥴러에 접근 불가능
  - 하나의 상태를 전역으로 관리하기 위해 스토어, 액션, 리듀서 등 많은 콛
 필요, Redux toolkit 사용하면 코드량 감소 가능
  - 단일 스토어는 하나의 자비스크립트 객체이므로 상태가 변경될 때마다 많은 컴포넌트들이 다시 렌더링됨
- Recoil
  - 인터페이스 간단
  - React에서 유지보수하는 라이브러리, React와 호환 좋음
  - 비동기 작업 처리와 상태 관리를 위한 긴단한 인터페이스

#### 자바스크립트 함수형 프로그래밍

- 자바스크립트는 멀티프로그래밍 패러다임을 지원
  - 명령형, 객체지향형, 함수형 프로그래밍
- 함수형 프로그래밍은 순수 함수(pure function)와 보조 함수의 조합을 통해 로직 내에 존재하는 조건문과 반복문을 제거하여 복잡성을 해결하고 변수의 사용을 억제하여 상태 변경을 피하려는 프로그래밍 패러다임
- 조건문이나 반복문은 로직의 흐름을 이해하기 어렵게 하여 가독성을 해치고, 변수는 누군가에 의해 언제든지 변경될 수 있어 오류 발생의 근본적 원인이 될 수 있기 때문임
- 함수형 프로그래밍은 결국 순수 함수를 통해 부수 효과를 최대한 억제하여 오류를 피하고 프로그램의 안정성을 높이려는 노력의 일환
- 어떻게 보다는 무엇을(What) 할 것인지에 포커스
- 대입문 사용하지 않음 (ex. for문의 int i = 0)
- 주요 키워드: 부수 효과(Side effect), 순수 함수, 1급 객체, 참조 투명성
- 함수는 1급 객체로서 취급
  - 즉, 함수를 파라미터로 넘기는 등의 작업 가능
- 부수 효과
  - 변수의 값이 변경됨
  - 자료 구조를 제자리에서 수정함
  - 객체의 필드값을 설정함
...
- 함수형 프로그래밍 예시
  - 자바 8의 Stream API
  - filter, map, collect 등과 같은 함수의 파라미터로 함수가 전달됨 (1급 객체)
  - 사용할 함수를 변수로 선언하는 것도 가능 (ex. Function<String, String> function = word -> word.toUpperCase();)
...
- 데이터바인딩
  ...
  - MVC 모델에서 model과 view를 서로 묶어 model과 view의 "자동 동기화" 시키기
- 양방향 데이터 바인딩
  ...
  - 컨트롤러에서 model이 변경됨 -> view변경됨 -> view에서 scope model이 변경됨 -> 컨트롤러에서 model이 변경됨
    - 컨트롤러와 뷰 양쪽의 데이터 일치가 모두 가능한 것이 양방향 데이터 바인딩
    ...
    - 입력된 값이나 변경된 값에 따라 내용이 바로 바뀌기 때문에 따로 체크해주지 않아도 됨
    - 양방향 데이터 바인딩은 웹 애플리케이션의 복잡도가 증가하면 증가할수록 빛을 발함
    - 수많은 코드의 양을 줄여줄 뿐만 아니라 유지보수나 코드를 관리하기 매우 쉽게 해줌
- 단방향 데이터 바인딩
  ...
  - 대표적으로 React가 단방향 데이터 바인딩
  - 단방향 데이터 바인딩은 데이터와 템플릿을 결합해 화면을 생성 (JS -> HTML만 가능)
  - 사용자의 입력에 따라 데이터를 갱신하고 화면을 업데이트 해야 하므로 단방향 데이터 바인딩으로 구성하면, 데이터의 변화를 감지하고 화면을 업데이트 하는 코드를 매번 작성 필요
  - 리액트는 자바스크립트 기반으로, 부모 뷰->자식 뷰 바뀐 내용을 직접 전달
...
- 자식 컴포넌트의 props나 state에 변경 사항이 있었느냐는 무관함
- 형제 관계인 컴포넌트들끼리는 서로 리렌더링에 영향을 미치지 않음
- shouldComponentUpdate에서 true가 반환될 때
  - 각각의 컴포넌트는 shouldComponentUpdate 라는 메소드를 가지고 있는데 이는 state가 변경되거나 새로운 props를 전달받는경우 실행
