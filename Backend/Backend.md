...
- (1) 사용자 입력
  - 사용자는 로그인 화면에서 아이디(이메일 또는 사용자명) 와 비밀번호를 입력
  - '로그인' 버튼 클릭 시 브라우저에서 이벤트 발생

- (2) 클라이언트 요청 처리
  - JS 코드에서 입력값 유효성 검사를 수행 (공백 체크, 이메일 형식 등)
  - 유효하면 HTTPS POST 요청으로 로그인 API 호출
  - 보통 JSON 형식으로 서버에 전송:
    ```json
    {
...
- (3) 서버 수신 및 인증 처리
  - 서버는 요청을 수신하고, 다음 과정을 거침:
    - (1) 요청 검증
      - 입력값의 유효성 재검사 (백엔드 단에서 반드시 재확인)

    - (2) 사용자 확인
      - DB에서 해당 이메일/아이디에 해당하는 사용자 존재 여부 확인

    - (3) 비밀번호 검증
      - 저장된 비밀번호는 일반적으로 해시 처리되어 있음 (예: bcrypt, scrypt)
      - 사용자가 입력한 비밀번호를 같은 방식으로 해시 후, 저장된 해시와 비교

    - (4) 인증 성공 시 처리
      - 인증이 성공하면 세션 또는 토큰(JWT 등) 생성

- (4) 서버 응답
  - 인증 성공:
    - 200 OK와 함께 토큰(JWT) 또는 세션 쿠키 전달
    - 추가적으로 사용자 정보(이름, 권한 등)도 함께 응답

  - 인증 실패:
    - 401 Unauthorized 또는 403 Forbidden 응답
    - 실패 사유(아이디 없음, 비밀번호 오류 등)는 최소한으로 노출

- (5) 클라이언트 처리
  - 응답 받은 토큰은 클라이언트(LocalStorage, Secure Cookie 등)에 저장
  - 이후 모든 API 요청 시 Authorization 헤더에 토큰 포함
  - 로그인 성공 시 홈 화면 또는 메인 페이지로 라우팅 전환

- 보안 고려사항
  - HTTPS 사용 필수 (패킷 가로채기 방지)
  - 비밀번호 평문 저장 금지 – 해시 & salt 적용
  - 토큰 탈취 방지 – 토큰 저장 위치 신중히 (쿠키 + HttpOnly 또는 Secure Storage 등)
  - Brute-force 방지 – 로그인 실패 횟수 제한, CAPTCHA 적용
  - CSRF/XSS 방어 – 토큰 사용 방식, CSP 설정 등

- 인증 방식 구분
  - 세션 기반 인증: 서버가 세션을 관리, 쿠키로 세션ID 전달
  - 토큰 기반 인증: 서버는 상태 유지 안함, 클라이언트가 토큰 보관 (JWT 등)
  - OAuth2 / OpenID Connect: 외부 인증 (구글/카카오 등)
